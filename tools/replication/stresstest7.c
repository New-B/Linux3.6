...
#define ARRAY_NB_ENTRIES      	(1024*1024*8)
#define PRIVATE_ARRAY_NB_ENTRIES	(100)
#define NR_ITERATIONS            10

static __attribute__((aligned(4096))) int array[ARRAY_NB_ENTRIES];
...
__attribute__((optimize("O0"))) void* read_a(void* core_nr)
{
   ...
   int pa[PRIVATE_ARRAY_NB_ENTRIES];

   int fake_sum = 0;
   for(int i=0; i<ARRAY_NB_ENTRIES; i++) {
      //printf("[CORE %lu] %d\n", _core_nr, array[i]);
      if(i<PRIVATE_ARRAY_NB_ENTRIES) {
         fake_sum += array[i] + pa[i];
         //fake_sum += pa[i];
      }
   }
...
int main()
{
   ...
   memset(array, 0, ARRAY_NB_ENTRIES*sizeof(int));
